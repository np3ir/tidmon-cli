...
- **Sync to Your TIDAL Favorites**: Push your monitored library to your account with `tidmon favorite` — follow every artist and add every playlist in one command (batched, idempotent). Use `tidmon follow` / `tidmon unfollow` (by ID, name, or `--file`) to curate exactly which artists occupy TIDAL's **10,000-favorite** cap. These are the only commands that write to your account.
